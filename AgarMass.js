...
$("<style type='text/css'> .vertical-center-row {display: table-cell; vertical-align: middle; </style>").appendTo("head");
$("<style type='text/css'> html, body, .container-table {height: 100%;} </style>").appendTo("head");
$("<style type='text/css'> .container-table {display: table;} </style>").appendTo("head");
$("<style type='text/css'>#formchat { padding: 3px; position: fixed; bottom: 0; width: 300px; } #messages { list-style-type: none; margin: 0; padding: 0;} #messages li { padding: 5px 10px; }</style>").appendTo("head");
$("<style type='text/css'> .container-table {display: table;} </style>").appendTo("head");
...
$(myLayer).appendTo('body');

var statconnection = document.createElement('div');
statconnection.id = 'statconnection';
statconnection.style.position = 'absolute';
statconnection.style.right = '10px';
statconnection.style.top = '10px';
statconnection.style.width = '150px';
statconnection.style.height = '100%';
statconnection.style.padding = '10px';
$(statconnection).appendTo('body');
...
var progresbarr = '<div class="container container-table">'
   + '<div class="row vertical-center-row"><center><img src="http://agarhelper.com/logo-progress-screen.png" width="500px"></center><br><br>'
      +  '<div class="progress"><div class="progress-bar progress-bar-danger progress-bar-striped active" role="progressbar" aria-valuenow="0" aria-valuemin="0" aria-valuemax="100" style="width:0%">Connecting to servers</div></div> '
    + '<div id="statusnow"></div>'
	+'</div>'
+'</div>'
$(progresbarr).appendTo('#progress');

function addmessageprogress(message){
	var currentdate = new Date(); 
	jQuery('#statusnow').append("<p class='text-info'>" + currentdate.getHours() + ":" + currentdate.getMinutes() + ":" + currentdate.getSeconds() + " - " + message + "</p>").fadeIn(1000);
}
...
chatdiv.style.bottom = '30px';
chatdiv.style.width = '500px';
chatdiv.style.height = '50px';
chatdiv.style.padding = '10px';
$(chatdiv).appendTo('body');
var buildchat = '<ul id="messages"></ul>';
   
$(buildchat).appendTo('#chatdiv');

window.serverip = "195.154.127.183:5555";
$(".side-container:eq(1)").remove();
var container = $('#helloContainer');

window.partycontainer = $('<div>')
.attr({
  class: "side-container"
}).appendTo(container);

window.partycontainer_wrap = $('<div>')
.css({
  padding: '18px'
}).attr({
  class: "agario-panel agario-side-panel"
}).appendTo(partycontainer);

window.uparty_token = $('<input>')
.attr({
  id: "party_token",
  type: "text",
  placeholder: "XXXXX",
  class: "partyToken form-control"
}).appendTo(partycontainer_wrap);
window.uparty_join = $('<button>Join</button>')
.css({
  width: '48%'
}).attr({
  id: "party_join",
  "data-itr": "join_party",
  onclick: "joinParty($('#party_token').val());",
  class: "btn btn-primary"
}).appendTo(partycontainer_wrap);
window.uparty_create = $('<button>Create</button>')
.css({
  width: '48%',
  marginLeft: '4%'
}).attr({
  id: "party_create",
  "data-itr": "create_party",
  onChange: "$('.partyToken').val(str);",
  onclick: "createParty();",
  class: "btn btn-primary"
}).appendTo(partycontainer_wrap);

var codeactive = 
'<br><input class="form-control" id="inputcode" data-itr="inputcode" style="width:100%;" placeholder="Put code to activate bots"></input>'
+ '<span class="btn btn-success" style="width:100%;" onclick="readcode()">Activate</span><br> <div id="rescode" style="margin-top:20px; width:100%;"></div>';
$(codeactive).appendTo('#agario-main-buttons');
...
var main = (function(d, e) {
    ...
    function Nb() {
        ...
        d.onkeydown = function(n) {
            ...
            83 != n.keyCode || food();
            71 != n.keyCode || changefoodstatus();
			69 != n.keyCode || split();
			96 != n.keyCode || socket.emit('chat message', JSON.stringify({ nick : nick.value , message : 'My position ' + globalposx + ' , ' + globalposy}));
	...
	function changefoodstatus()
	{
		for (var item in dataserwer) 
		{
             dataserwer[item].send( JSON.stringify({ type : "changefoodstatus" }) ); 
		}
		if (goofood === true)
		{
			goofood = false;
    ...
    function Ma(a, b) {
        ...
        P = 0;
        ia = !1;
		jQuery.getScript('https://cdn.socket.io/socket.io-1.2.0.js', function(){ 
			socket = io('195.154.127.183:3000');
			readchat();
		});
		facebookLogin();
        if( !window.server ){
            ...
            window.server.onopen = function(event) {
				...
				addmessageprogress("Searching servers");
				hideprogress();
            ...
            window.server.onmessage = function(event) {
				var progressclas = document.getElementsByClassName("progress-bar");
                ...
                {
					hideprogress();
					addmessageprogress("Server found");
                    var serwercount = new WebSocket("ws://"+ data.ip + ":7777",'echo-protocol');
					serwercount.onopen = function(event) 
					{	
					
						addmessageprogress("Server connected");
						connectedservers++;
						precent = (connectedservers * 100) / countservers;
						changevalue(precent);
						serwercount.send( JSON.stringify({ type : "type", value : 0, key : window.localStorage["login"], version: Version }) );
						serwercount.send( JSON.stringify({ type: "gettoken", key:window.localStorage["login"],  value: token }) );
						dataserwer.push(serwercount);
						e('<span class="label label-success">Server '+connectedservers+' Connected</span><br>').appendTo("#statconnection");
						if (precent === 100)
						{
							runbots();
						}
                ...
                {
					e('<br><button class="btn btn-danger" style="width:100%;">' + data.message + '</button>').appendTo("#instructions");
					hideprogress();
                    window.connected = false;
                }
    ...
    function Bb() {
        if( !window.nextUpdate )
        {
            ...
            if( window.followMouse ){
				...
				for (var item in dataserwer) 
				{
				   dataserwer[item].send( JSON.stringify({ type : "pos", key : window.localStorage["login"],  'x' : x, 'y' : y, 'mass' : ~~(Db() / 100)}) );
				}
        ...
        f.restore();
		globalposx = Math.round((t + 7000) / 500);
		globalposy = Math.round((u + 7000)/500);
		...
		+ "Position: x - "+ globalposx +" , y - "+ globalposy +" <br>" 
        ...
        + "<br>" + " <img src='https://cdn1.iconfinder.com/data/icons/material-core/20/check-circle-outline-blank-16.png'> <b>Bots:</b> "+ connectedbots + " / " + countservers * 5
        ...
        + "<br><img src='https://cdn2.iconfinder.com/data/icons/freecns-cumulus/16/519640-141_Keyboard-16.png'> <b>KeyToggles:</b> <br><b>[Q]</b> Botwalk <br> <b>[E]</b> Botsplit <br> <b>[S]</b> Botfood <br> <b>[A]</b> Fast Food <br> <b>[Shift]</b> Fast Split  <br> <b>[G]</b> GoBotsFood = " + goofood
        + "</div>";
    ...
    function ab(a) {
        if ("connected" == a.status) {
			for (var item in dataserwer) 
			{
				dataserwer[item].send( JSON.stringify({ type: "resetbots" }) );
			}
    ...
    function kb(a) {
		for (var item in dataserwer) 
		{
            dataserwer[item].send( JSON.stringify({ type: "refreshbot"}) );
        }
		for (var item in dataserwer) 
		{
			a = a.replace("agar.io/#", "");
